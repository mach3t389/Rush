...
**Synchronisation Google Calendar** (`app/api/google-calendar-sync.ts`) — un seul endpoint, trois appelants distingués par leur en-tête `Authorization` :

| Sens | Déclencheur | Fréquence |
|------|-------------|-----------|
| Rush → Google (`handlePush`) | `eventStore.ts` à chaque create/update/delete | immédiat |
| Google → Rush, tous les studios (`handlePull`) | cron Vercel (`app/vercel.json`), en-tête `Bearer CRON_SECRET` | **1×/jour** — plafond du plan Hobby |
| Google → Rush, un seul studio (`handleUserPull`, `action: 'pull'`) | `pullFromGoogleCalendar()` dans `eventStore.ts`, au montage de `Dashboard`, `CalendrierGlobal`, `ProjetCalendrier` | à l'ouverture, limité à 1× / 2 min (`sf_gcal_last_pull` dans `localStorage`) |

Les deux sens pull partagent `pullStudio()` (calendrier par défaut de l'organisation + un calendrier par projet actif). Le pull écrit en base avec la clé service-role, donc le cache mémoire d'`eventStore` ne le voit pas : `pullFromGoogleCalendar()` refetch explicitement après coup. Le garde-fou anti-spam est en `localStorage` (pas une variable de module) pour qu'un rechargement de page ou un 2ᵉ onglet ne relance pas un balayage qui vient de tourner ; il est marqué **avant** la requête pour qu'un échec ne boucle pas. Sessions démo : `pullFromGoogleCalendar()` ne fait rien. Studio sans Google connecté : bloqué par le cache de `getGoogleCalendarStatus()`, aucun appel à l'endpoint. **Un cron externe (cron-job.org, `pg_cron`) peut appeler le même endpoint avec `Bearer CRON_SECRET` aussi souvent que voulu** — c'est le contournement du plafond Hobby, aucun code à changer.
