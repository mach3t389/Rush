...
export function sendTeamInvitationEmail(email: string, role: string, link: string): void {
  ...
  void sendEmail(
    ...
    wrapEmailHtml(
      `<p>Bonjour,</p>
      <p><strong>${studioName}</strong> vous invite à rejoindre son équipe sur Rush en tant que <strong>${role || 'membre'}</strong>. Rush est une plateforme de gestion de projets utilisée par ${studioName} pour organiser son travail, ses fichiers et ses échanges avec ses clients.</p>`,
