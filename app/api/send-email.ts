...
async function handleDigestRun(_req: VercelRequest, res: VercelResponse) {
  ...
  for (const row of dueUsers) {
    ...
    const appLink = process.env.VITE_APP_URL ?? 'https://rushflow.app';
    const html = `<div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; color: #1a1a1a; line-height: 1.5;">
      <p style="font-weight: 700; font-size: 15px; letter-spacing: -0.2px; margin-bottom: 20px;">Rushflow</p>
      <p>Voici votre récap quotidien.</p>
      <p>Depuis votre dernier récap, vous avez reçu : ${parts.join(', ')}.</p>
      <p style="margin: 24px 0 8px;">
        <a href="${appLink}" style="display: inline-block; padding: 10px 20px; background: #f9ff00; color: #14140a; text-decoration: none; border-radius: 8px; font-weight: 600;">Voir le détail dans Rushflow</a>
      </p>
